...
For mixed human-robot tracking, this channel can be used in 3 compact ways. First, reflectivity can be combined with height filtering to suppress unreliable ground reflections and emphasize target-level returns. Second, reflectivity descriptors can be combined with KNN-style neighborhood matching or state estimation to improve target consistency when geometry is ambiguous. Third, corrected reflectivity can be mapped into grayscale pseudo-images, allowing lightweight visual-style features to support classification and re-identification. Therefore, reflectivity is a useful candidate for the Reflectivity Voter, especially when close targets have similar shapes but different materials.
\end{contentbox}
...
Instead of using 1 fixed radius in all directions, the human target can be represented by an elliptical effective boundary whose orientation is inferred from two coupled cues: the LiDAR-facing visibility direction and the target's motion direction. This is important for LiDAR-only tracking because ray-casting mainly samples the visible side of the body, so a person may appear as a crescent-like or partial elliptical point cloud rather than a complete circle or square. Therefore, the key idea is not only to fit an ellipse, but to extract orientation cues from this anisotropic, incomplete local structure and recover a more reliable target direction and boundary during merging and occlusion.
