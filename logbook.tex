...
\boxitemtitle{AQ4: Which Tracking Model Is Suitable under Occlusion?}
\begin{contentbox}
The tracking-model test uses the same 2-target LiDAR simulation family as the main report. Two rounded human targets move through crossing and turning cases, the ceiling-mounted LiDAR is placed either at the left side or bottom side of the room, and DBSCAN provides the frame-wise measurements for a Kalman-filter tracker. The purpose is to identify when a simple tracking model is still useful and when it becomes the main source of ID error.

\begin{center}
    \includegraphics[width=0.95\linewidth]{Figure8.png}
    \captionof{figure}{Tracking-model stress test under multi-target crossing and turning motion.}
    \label{fig:aq4-tracking-model-overview}
\end{center}

The result in Figure~\ref{fig:aq4-tracking-model-overview} shows that a standard constant-velocity KF is acceptable for short, smooth motion gaps, but it becomes fragile near intersections. At close range, clustering may merge the two point clouds, so the tracker receives either an ambiguous centroid or no reliable measurement.

\begin{center}
    \includegraphics[width=0.95\linewidth]{Figure10.png}
    \captionof{figure}{Compact summary of KF prediction behavior during the high-risk crossing interval.}
    \label{fig:aq4-kf-stress-summary}
\end{center}

Figure~\ref{fig:aq4-kf-stress-summary} further supports the same conclusion: prediction can bridge a short missing interval, but long merge or occlusion periods require stronger association logic. EKF, IMM, particle filtering, or GMM-based tracking should be compared when the motion contains turning, stopping, or multiple possible future positions.
\end{contentbox}
