...
\begin{contentbox}
In real tracking scenarios, merging and occlusion often occur together. To study this combined failure mode, I built an X-type crossing model focused on the high-risk intersection where both point-cloud merging and occlusion can occur. Figure~\ref{fig:x_crossing} shows the accumulated point-cloud and evaluation results. Red and blue indicate the two different targets, while yellow indicates merged or ambiguous points where the two identities cannot be clearly separated. A triangle above a target indicates that the target is occluded, and the triangle size represents the occlusion ratio. The star marks the LiDAR position. Because this situation is strongly affected by viewpoint, the X-type motion is tested from two LiDAR positions. The last case is the only ID-loss case, and it exposes a limitation of the KF: after the targets merge in the high-risk area, the tracker can lose the turning direction information. The second row gives the evaluation more directly through occlusion ratio and tracking quality. The quality score is based on point density, so it is affected by both LiDAR-target distance and occlusion. Additional ground-truth trajectories, calibrated tracking trajectories, and oblique 3D accumulated point-cloud views are provided in AQ4.
...
\boxitemtitle{AQ4: Additional Graphs of X-Crossing Testing}
\begin{contentbox}
Figure~\ref{fig:aq4-tracking-model-overview} and Figure~\ref{fig:aq4-kf-stress-summary} provide supplementary views of the X-crossing experiments. They show the trajectory and RMSE analysis, together with oblique 3D accumulated point-cloud views and top-view summaries of the simulated two-person walking cases.

\begin{center}
    \includegraphics[width=0.95\linewidth]{Figure8.png}
    \captionof{figure}{Supplementary trajectory and RMSE analysis for the X-crossing test cases.}
    \label{fig:aq4-tracking-model-overview}
\end{center}

The scenario set contains three main motion types. In the parallel walking case, two people move with the same speed. Occlusion can still appear because both targets are located on the same side of the LiDAR; this simple case checks whether the KF can maintain prediction under weak ambiguity. In the X-straight cases, shown as the 2nd and 3rd columns, target A waits for nearly 1 second so that the closest distance is about $0.52~\mathrm{m}$, still larger than $2R=0.50~\mathrm{m}$. In the X-turn cases, the model includes acceleration, a circular turning radius around the joint point, and short acceleration phases at the start and end of the motion.

\begin{center}
    \includegraphics[width=0.95\linewidth]{Figure10.png}
    \captionof{figure}{Oblique 3D accumulated point-cloud view and top-view summary for the X-crossing cases.}
    \label{fig:aq4-kf-stress-summary}
\end{center}

These results show that LiDAR viewpoint is important. The same motion model can produce different merging and occlusion directions when observed from different sensor positions, so the judgement result may also change. Turning inside the high-risk region can remove direction information after merging, which suggests that future work should use additional independent signatures such as height, reflectivity, or other stable LiDAR-intrinsic features. Merging and occlusion are physically different: occlusion is mainly radial and front-back, while merging is caused by lateral distance shrinkage across the illuminated boundary. However, in complex crossing cases, both effects can happen at the same time.
\end{contentbox}
\boxitemtitle{AQ5: What Do the Extended X-Turn Experiments Reveal?}
\begin{contentbox}
This experiment focuses on the X-turn-bottom case. Two targets approach the crossing region, make nonlinear turns, and pass through the closest-distance interval under an OSDome-like LiDAR at $H=2.7~\mathrm{m}$. The setup uses 50 frames at $0.1~\mathrm{s}$ per frame, target radius $0.25~\mathrm{m}$, range-dependent sparsity, gait oscillation, and DBSCAN with $\epsilon=0.22~\mathrm{m}$ and $MinPts=14$.

\begin{center}
    \includegraphics[width=0.95\linewidth]{Figure11.png}
    \captionof{figure}{X-turn trajectory response showing KF prediction error around the nonlinear maneuver.}
    \label{fig:aq5-xturn-kf}
\end{center}

Figure~\ref{fig:aq5-xturn-kf} shows the main limitation of a purely linear KF. The constant-velocity assumption predicts forward inertia, so the estimated state can lag or overshoot when the target turns sharply. This problem is most visible near the crossing point, where the measurements are also least reliable because the two targets are close.

\begin{center}
    \includegraphics[width=0.95\linewidth]{Figure12.png}
    \captionof{figure}{Visibility and point-quality changes during the same close-interaction experiment.}
    \label{fig:aq5-visibility-quality}
\end{center}

Figure~\ref{fig:aq5-visibility-quality} explains why the tracking failure is not only a filter problem. The point cloud depends strongly on the LiDAR viewpoint. A ceiling or top-side view can preserve more planar separation, while a side or bottom view can create larger geometric shadows. When one target blocks the other, the rear target receives fewer valid points and the measurement confidence drops.

\begin{center}
    \includegraphics[width=0.95\linewidth]{Figure13.png}
    \captionof{figure}{3D tracker-assigned point clouds showing view-dependent sparsity, shadows, and merged states.}
    \label{fig:aq5-3d-viewpoint}
\end{center}

Figure~\ref{fig:aq5-3d-viewpoint} summarizes the system-level result. LiDAR does not observe the full object surface; it mainly sees the sensor-facing side and misses regions behind the first visible target. Therefore, the tracker should treat LiDAR placement as a design variable. The tracking model, clustering parameters, and 3-Voter state logic should be tuned together for the chosen installation viewpoint.
\end{contentbox}
...
Figure~\ref{fig:aq7-engineered-buffer} compares the baseline with the 3 active models. The result shows stable behavior under the tested settings and random structural noise ($\sigma = 0.015\text{ m}$). The active models increase the bubble gap, which means DBSCAN merging is delayed.
