...
\\
The anisotropy test compares 1 baseline with 3 buffer-strengthening models. The baseline is the normal range-decay simulation and is used only for comparison. The 3 active models match the labels in \textbf{Figure 7}: \textbf{Power-law}, \textbf{Sigmoid}, and \textbf{Density}.
...
Figure 7 compares the baseline with the 3 active models. The result shows stable behavior under the tested settings and random structural noise ($\sigma = 0.015\text{ m}$). The active models increase the bubble gap, which means DBSCAN merging is delayed.
