...
A LiDAR emits beams at fixed angular increments: horizontal $\Delta\theta$ and vertical $\Delta\phi$. At distance $d$, the local spherical footprint expands as
\[
...
\begin{contentbox}
This appendix item provides supplementary views for the X-crossing experiment in Section 2.4. Figure~\ref{fig:aq4-tracking-model-overview} focuses on the true motion and the calibrated KF tracking result. Row 1 shows the ground-truth motion models for the 5 test cases. In the parallel case, the 2 targets use identical object models and move with the same speed. All cases include short acceleration and deceleration phases at the beginning and end, while the middle part of each route is approximately uniform-speed motion. In columns 2 and 3, the targets move straight through the X-shaped crossing. To keep the motion physically valid, target A waits for about $1~\mathrm{s}$, so the 2 targets do not overlap; the closest distance is about $0.52~\mathrm{m}$, which is still larger than the radius sum $2R=0.50~\mathrm{m}$. In columns 4 and 5, the targets turn near the crossing focus. These turns include centripetal acceleration and a small circular arc, so the trajectories remain physically plausible.
...
Figure~\ref{fig:aq4-kf-stress-summary} gives an oblique 3D accumulated point-cloud view, making the 2 walking trajectories and the LiDAR-view dependence easier to observe. This experiment studies a small set of variables, but it already shows that the same motion model can produce different tracking outcomes from different LiDAR viewpoints. The result changes when merged points are visible from 1 side, or when occlusion removes too many points from another side.
