...
\begin{contentbox}
In real tracking scenarios, merging and occlusion do not appear as two isolated phenomena. They can occur separately, overlap for a short interval, or interact in a more complex way when targets enter the same high-risk region. To study this behavior, I simulated an X-shaped two-person model, as shown in Figure~\ref{fig:x_crossing}. The 1st column is a parallel-walking case, where the yellow star marks the LiDAR position. The 2nd and 3rd columns are straight X-crossing cases observed from different LiDAR viewpoints, and the 4th and 5th columns are X-turn cases where both targets turn near the crossing focus. In the accumulated point-cloud row, red and blue indicate the two identities, yellow indicates merged or ambiguous points, and the triangle markers indicate occluded centroids with marker size related to the occlusion ratio. The lower row describes visibility and tracking-quality features. Since the quality score is based on point density, it is jointly affected by LiDAR-target distance and by occlusion. Among these cases, only the 5th case produces an ID loss/swap, although the tracker still keeps an approximate position. The most direct conclusion is that when turning occurs inside the high-risk region, a simple KF prediction can lose the turning-direction information after merging. More details, including ground-truth trajectories, accumulated point clouds, calibrated tracking trajectories, and oblique 3D accumulated point-cloud views, are provided in AQ4.
