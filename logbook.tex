...
\begin{titlepage}
    ...
    \begin{flushleft}
        \begin{tabular}{ll}
            ...
            \textbf{Course code:} & 5I010 Internship EE 10 ects
        \end{tabular}
    \end{flushleft}
...
The framework contains 3 main modules. 
\begin{itemize}
    \item  \textbf{point-cloud generation} module produces simplified human targets using a ceiling-mounted OSDome-like LiDAR model. It considers body geometry, a rounded head--shoulder cap, gait oscillation, range noise, distance-dependent sparsity, and visibility-dependent point distribution. 

\item  \textbf{clustering and trajectory prediction} module estimates target positions frame by frame from the generated point clouds. This module is independent from the ground-truth trajectory, so the tracker does not directly know the true target center. 

\item  \textbf{The evaluation} module compares tracking performance based on criteria such as ID loss, merged-frame ratio, and trajectory quality.
\end{itemize}
...
\vspace*{0.5em}
By actively exploiting this property and strengthening sparsity, the system can enlarge this ``air buffer''. Such a strategy may act as a geometric protective mechanism under severe spatial proximity. More testing groups of strengthening results are listed in AQ
\end{minipage}%
\hfill
...
\captionof{figure}{Simulation of the margin gap along a V-shaped trajectory, illustrating the theoretical merge threshold versus the actual sparse-point merge event. (Triangles indicate the LiDAR position at the height of 2.7m)}
...
Second row & occlusion ratio, density-based quality, and merge flag \\
\hline
\end{tabular}
\end{minipage}
...
For mixed human-robot tracking, this channel can be used in 3 compact ways: 


1) reflectivity can be combined with height filtering to suppress unreliable ground reflections and emphasize target-level returns; 


2) reflectivity descriptors can be combined with KNN-style neighborhood matching or state estimation to improve target consistency when geometry is ambiguous; and 


3) corrected reflectivity can be mapped into grayscale pseudo-images, allowing lightweight visual-style features to support classification and re-identification. Therefore, reflectivity is a useful candidate for the Reflectivity Voter, especially when close targets have similar shapes but different materials.
\end{contentbox}
